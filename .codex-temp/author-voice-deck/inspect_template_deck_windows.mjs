...
import { importRuntimeModule } from "file:///C:/Users/ytcon/.codex/plugins/cache/openai-primary-runtime/presentations/26.813.12317/skills/presentations/container_tools/runtime_helpers.mjs";
...
function runCapture(command, args, options = {}) {
  const isWindowsUnzip = command === "unzip" && process.platform === "win32";
  const actualCommand = isWindowsUnzip ? "powershell.exe" : command;
  const actualArgs = isWindowsUnzip
    ? ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", process.env.CODEX_UNZIP_PS1, ...args]
    : args;
  ...
  if (result.status !== 0) {
    ...
    throw new Error((stderr || stdout || result.error?.message || `${actualCommand} ${actualArgs.join(" ")} failed`).trim());
  }
